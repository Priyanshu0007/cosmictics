...
import CartStardustCanvas from './CartStardustCanvas';
...
const Cart = ({ setShowCart }: any) => {
    ...
    return (
        <div className='bg-black/70 backdrop-blur-sm w-full min-h-screen fixed inset-0 z-50 transition-opacity duration-300 flex justify-end'>
            {/* Drawer Container */}
            <div className='max-w-[420px] w-full min-h-full bg-obsidian-light border-l border-white/10 p-6 flex flex-col justify-between shadow-2xl animate-slide-left relative overflow-hidden'>
                <CartStardustCanvas />
                
                <div className="relative z-10 flex flex-col justify-between h-full flex-grow">
                    <div>
                        <button 
                            className="absolute right-0 top-0 text-gray-400 hover:text-white transition-colors duration-200 cursor-pointer"
                    ...
                    <div className='space-y-4 border-t border-white/5 pt-6 bg-transparent'>
